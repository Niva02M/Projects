...
export const TextRevealCard = ({ text, revealText, children, className }) => {
  ...
  return (
    <div
      ...
      ref={cardRef}
      className={cn(
        "bg-transparent rounded-lg relative overflow-hidden px-2",
        className
      )}
    >
      {children}
      <div className="h-40 relative flex items-center overflow-hidden">
        <motion.div
          ...
          className="absolute z-20 bg-gradient-to-t from-black via-black to-transparent will-change-transform rounded-2xl shadow-2xl"
        >
          <p
            style={{ textShadow: "4px 4px 15px rgba(0,0,0,0.5)" }}
            className="text-sm sm:text-[4rem] py-10 font-bold text-white bg-clip-text text-transparent bg-gradient-to-b from-white to-neutral-300"
          >
            {revealText}
          </p>
        </motion.div>
        ...
        <div className="[mask-image:linear-gradient(to_bottom,transparent,white,transparent)]">
          <p className="text-sm sm:text-[4rem] py-10 font-bold gont-bebas bg-clip-text text-transparent bg-[#323238]">
            {text}
          </p>
          <MemoizedStars />
        </div>
